...
final class Util {
	 ...
	 * @param Array $viewData contains the data which want to convert in XML
	 * <p>This function converts the array data in XML format</p>
	 * @return void
	 */
	public static function printArr($viewData) {
		echo '<pre>';
		print_r($viewData);
		echo '</pre>';
	}
	 ...
	 * @param Array $viewData contains the data
	 * @param String $viewFile contains the name of viewfile
	 * <p>This function transform the user on to specified view file</p>
	 * @return void
	 */
	public static function renderView($layout = '', $view = '', array $viewData = array()) {
		if(isset($_GET['debug'])) {
			if($_GET['debug'] == 1) {
				self::printArr($viewData);
            }
			exit;
		}
		if(strlen($layout) < 1) {
			$layout = 'default.view.php';
		}
		header("Content-type: text/html; charset=utf-8");
        if($layout == '') { // only $view
            include_once('view/'.$view.'.view.php');
        } else if($layout != '') {
            include_once('view/layouts/'.$layout.'.view.php');
        } else {
            echo "View Missing";
        }
	}

    public static function renderViewAjax($viewFile = '', array $viewData = array()) {
        if(isset($_GET['debug'])) {
			if($_GET['debug'] == 1) {
				self::printArr($viewData);
            }
    ...
    public static function renderViewEmail($emailFile = '', array $viewData = array()) {
		if(isset($_GET['debug'])) {
			if($_GET['debug'] == 1) {
				self::printArr($viewData);
            }
	 ...
	 * @return void
	 */
    public static function renderJson(array $viewJsonData = array()) {
		if(isset($_GET['debug'])) {
			if($_GET['debug'] == 1) {
				self::printArr($viewJsonData);
            }
			exit;
		}
		header("Content-type: application/json");
        echo json_encode($viewJsonData);
	}
	 ...
	 * @return void
	 */
    public static function renderXml(array $viewXmlData = array()) {
		if(isset($_GET['debug'])) {
			if($_GET['debug'] == 1) {
				self::printArr($viewXmlData);
            }
			exit;
		}
	 ...
	 * @return void
	 */
    public static function renderViewCsv($viewFile = '', array $viewCsvData = array()) {
		if(isset($_GET['debug'])) {
			if($_GET['debug'] == 1) {
				self::printArr($viewCsvData);
            }
			exit;
		}
		if(strlen($viewFile) < 1) {
		...
		header("Content-disposition: attachment; filename=" . $viewData['filename']);
		include_once('view/'.$viewFile.'.view.php');
	}
	...
	public static function sendEmail($to, $toName, $from, $fromName, $subject, $mailContent) {
        ...
        $headers .= 'X-Mailer: Example/' . phpversion() . PHP_EOL;
    ...
    public static function cleanInputData($string) {
