...
class DbOperations {
	...
	public function getObject($database) {
        $this->connection = DbConnection::getDBObject($database);
        global $databases;
        if(isset($databases[$database])) {
            $this->driver = $databases[$database]['driver'];
        }
	}
	 ...
	 * @param String $sqlOrCollection containing the query string which executes to retrive data
	 ...
	 * @return Array
	 */
	protected function select($sqlOrCollection, $params = array(), $database = DB1) {
		$resultArr = array();
		$this->getObject($database);
        if($this->driver == 'mongo') { // mongo
            $this->getObject($database);
            if($this->connection) {
                $collection = $this->connection->selectCollection($sqlOrCollection);
            ...
            $bindParams[0] = '';
            $i = 0;
            $statement = $this->connection->prepare($sqlOrCollection);
	 ...
	 * @return INT [Number of affected_rows]
	 */
	protected function delete($sqlOrCollection, $params = array(), $database = DB1) {
		$resultArr = array();
        $this->getObject($database);
        if($this->driver == 'mongo') { // mongo
            $this->getObject($database);
            ...
            if(DEBUG == 1 && !$statement) {
                error_log($this->getError() . ' :: ' . $sqlOrCollection);
            }
	 ...
	 * @param String $sqlOrCollection containing the query string which executes to insert data
	 * @param Array $params is array variable which stores parameter types
	 * @param Array $database contains database details
	 * <p> This function inserted the data in database table by calling prepare & execute method</p>
	 * @return INT [Number of affected_rows]
	 */
	protected function insert($sqlOrCollection, $params = array(), $database = DB1) {
		$insertID = 0;
        $this->getObject($database);
        if($this->driver == 'mongo') { // mongo
            $this->getObject($database);
            if($this->connection) {
                $collection = $this->connection->selectCollection($sqlOrCollection);
                // $params['_id'] = new MongoId(); // this is done automagically by Mongo
                $result = $collection->insert($params);
                if($result) {
                    $insertID = $params['_id']->{'$id'};
                }
            }
        } else { // mysql
            $success = false;
